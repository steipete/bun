...
describe("ws top-level TLS options", () => {
  ...
  // The TLS option checks must not route through user-overridable globals.
  // (Array.isArray is left alone: the pre-existing protocol parsing at the top
  // of the constructor still calls it.)
  it("still forwards options after Array.prototype.includes and ArrayBuffer.isView are clobbered", async () => {
    const out = await runInChild(`${childPreamble}
      Array.prototype.includes = () => { throw new Error("clobbered"); };
      ArrayBuffer.isView = () => { throw new Error("clobbered"); };
      console.log(await connect({ rejectUnauthorized: false, key: [tls.key], cert: tls.cert }));
      server.stop(true);
    `);
