...
`,
  ...
  describe("Bun.write(path, response) streams the body to the file", () => {
    ...
    it("a direct stream whose pull() rejects rejects with that error", async () => {
      using dir = tempDir("bun-write-direct-reject", {});
      const dest = join(String(dir), "out.txt");
      const stream = new ReadableStream({
        type: "direct",
        async pull(c) {
      ...
      await expect(Bun.write(dest, new Response(stream))).rejects.toThrow("boom");
    });
