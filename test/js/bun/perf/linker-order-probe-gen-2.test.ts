// Temporary probe, part 2: more in-worker rounds of the tracer+pty case pair
...
test.skipIf(!isLinux || isMusl || process.arch !== "arm64")(
  "the tracer case inside parallel workers, part 2",
  async () => {
    const { stalls, staleReports } = await runGeneratedRounds({ label: "gen2", budgetMs: 150_000 });
