    ...
    NewRequestContext<Self, SSL, DEBUG, true>: super::request_context::RequestContextHostFns,
{
    ...
    pub(crate) fn on_upgrade(
        ...
        // handler reads and edits. The raw uws request sees only the first
        // field of a repeated name and still sees a name the handler deleted,
        // so mixing the two answers a different request than the handler saw.
        if !request.has_fetch_headers() {
            // SAFETY: upgrader_ptr is live (ref_() above)
            let upgrader = unsafe { &*upgrader_ptr };
            let Some(req_ptr) = upgrader.req.get() else {
                return Ok(JSValue::FALSE);
            };
            ...
            // Keeps `if (server.upgrade(req)) return;` allocation-free for plain
            // requests. A first field that is absent or empty can never join
            // into a valid 24-byte key, so this loses no handshake.
            if r.header(b"sec-websocket-key").is_none() {
                return Ok(JSValue::FALSE);
            }
        ...
        let Some(head) = crate::webcore::body::BodyMixin::get_fetch_headers(request) else {
            return Ok(JSValue::FALSE);
        };
        {
