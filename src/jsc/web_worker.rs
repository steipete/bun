...
pub struct WebWorker {
    ...
    /// The worker is exiting on its own: `process.exit(code)` ran, or an
    /// uncaught error ended it. The exit code is decided; later paths must not
    /// overwrite it.
...
impl WebWorker {
    ...
    fn shutdown(&self) {
        ...
        // `on_exit` marks the VM shutting down once they ran, as on the main
        // thread: an 'exit' listener that throws is still the worker's error to
        // report (`uncaught_exception` reports nothing for a VM shutting down).
...
fn on_unhandled_rejection(
    ...
    //
    // Instead this is the worker's own `process.exit()` with the code as it now
    // stands (Node's handler ends the same way): it requests the stop and we
    // unwind to `spin()`'s `shutdown()`. Being the worker's exit, a
    // `process.exit(n)` that an error interrupted (its 'exit' listener threw)
    // keeps `n` rather than reading as a failed start.
    worker.exit();
}
