...
{
    if (isAbsolutePath(resolved) && (suffix.startsWith('#') || resolved.find('?') != WTF::notFound || resolved.find('#') != WTF::notFound))
        return makeString(URL::fileURLWithFileSystemPath(resolved).string(), suffix);
    return makeString(resolved, suffix);
}
    ...
    JSValue referrer, RefPtr<JSC::ScriptFetcher>, bool)
{
    ...
    Zig__GlobalObject__resolve(&res, globalObject, &keyZ, &referrerZ, &queryZ, splitSuffix);
    RETURN_IF_EXCEPTION(scope, {});
    if (!res.success) {
    ...
    bool deferred)
{
    ...
    if (sourceURL.isEmpty()) {
        ...
        auto suffix = fileURLSuffix(sourceURL);
        auto referrerKey = JSC::Identifier::fromString(vm, resolvedModuleKey(sourceOriginStringHolder, suffix));
...

    {
        ...
        BunString queryZ = BunStringEmpty;
        Zig__GlobalObject__resolve(&res, globalObject, &moduleNameZ, &sourceOriginZ, &queryZ, splitSuffix);
